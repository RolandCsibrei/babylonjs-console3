...
interface Console3CustomOptions {
  ...
  observerPanelColor?: BABYLON.Color3;
  debuggerHitPanelColor?: BABYLON.Color3;
  highliteHitPanelColor?: BABYLON.Color3;
...
class Console3LoggedEntity {
  public textInputs: BABYLON_GUI.InputText[] = [];
  ...
  public getLinkedMesh() {
    return this.getObject<BABYLON.AbstractMesh>();
  }
...
class console3 {
  ...
  private _entityTypeMappings = new Map<
    Console3LoggedEntityType,
    Console3LoggedEntityTypeMapping
  >();

  private _mainPanel!: BABYLON_GUI.StackPanel;
  private _consoleLinesPanel!:
    | BABYLON_GUI.StackPanel
    | BABYLON_GUI.ScrollViewer;
  private _buttonPanel!: BABYLON_GUI.StackPanel;
  private _scrollViewer!: BABYLON_GUI.ScrollViewer;
  ...
  constructor(
    ...
    options?: Console3CustomOptions,
    private _gui?: BABYLON_GUI.AdvancedDynamicTexture
  ) {
  ...
  private _createMappings() {
    const entityTypeMappings = new Map<
      Console3LoggedEntityType,
      Console3LoggedEntityTypeMapping
    >();
  ...
  private _drawMesh(entity: Console3LoggedEntity) {
    if (entity.textInputs) {
      const obj = console3._getObject<BABYLON.AbstractMesh>(entity);
      entity.textInputs[0].text = `${obj.position.x.toFixed(
        this._options.floatPrecision
      )}, ${obj.position.y.toFixed(
        this._options.floatPrecision
      )}, ${obj.position.z.toFixed(this._options.floatPrecision)} `;
      entity.textInputs[1].text = `${obj.rotation.x.toFixed(
        this._options.floatPrecision
      )}, ${obj.rotation.y.toFixed(
        this._options.floatPrecision
      )}, ${obj.rotation.z.toFixed(this._options.floatPrecision)} `;
      entity.textInputs[2].text = `${obj.scaling.x.toFixed(
        this._options.floatPrecision
      )}, ${obj.scaling.y.toFixed(
        this._options.floatPrecision
  ...
  private _drawVector2(entity: Console3LoggedEntity) {
    if (entity.textInputs) {
      const obj = console3._getObject<BABYLON.Vector2>(entity);
      entity.textInputs[0].text = `${obj.x.toFixed(
        this._options.floatPrecision
      )}, ${obj.y.toFixed(this._options.floatPrecision)}`;
    }
  }

  private _drawVector3(entity: Console3LoggedEntity) {
    if (entity.textInputs) {
      const obj = console3._getObject<BABYLON.Vector3>(entity);
      entity.textInputs[0].text = `${obj.x.toFixed(
        this._options.floatPrecision
      )}, ${obj.y.toFixed(this._options.floatPrecision)}, ${obj.z.toFixed(
        this._options.floatPrecision
      )}`;
    }
  }
  ...
  private _drawColor3(entity: Console3LoggedEntity) {
    if (entity.textInputs) {
      const obj = console3._getObject<BABYLON.Color3>(entity);
      entity.textInputs[0].text = `${obj.r.toFixed(
        this._options.floatPrecision
      )}, ${obj.g.toFixed(this._options.floatPrecision)}, ${obj.b.toFixed(
        this._options.floatPrecision
      )}`;
  ...
  private _drawColor4(entity: Console3LoggedEntity) {
    if (entity.textInputs) {
      const obj = console3._getObject<BABYLON.Color4>(entity);
      entity.textInputs[0].text = `${obj.r.toFixed(
        this._options.floatPrecision
      )}, ${obj.g.toFixed(this._options.floatPrecision)}, ${obj.b.toFixed(
        this._options.floatPrecision
  ...
  private _offsetInPixelsIfSceneExplorer() {
    const sceneExplorerHost = document.getElementById("BABYLON.sceneExplorer");
  ...
  private _createConsolePanel() {
    if (this._gui) {
      const scrollViewer = new BABYLON_GUI.ScrollViewer();
      ...
      const consolePanel = new BABYLON_GUI.StackPanel();
  ...
  private _createButtons() {
    const buttonPanel = new BABYLON_GUI.StackPanel("console3-button-panel");
  ...
  private _createInspectorButton(parent: BABYLON_GUI.StackPanel) {
    const btn = BABYLON_GUI.Button.CreateSimpleButton(
      "console3-button-inspector-toggle",
      "D"
    );
  ...
  private _createConsoleLinesToggleButton(parent: BABYLON_GUI.StackPanel) {
    const btn = BABYLON_GUI.Button.CreateSimpleButton(
      "console3-buttton-console-lines-toggle",
      "C"
    );
    btn.widthInPixels = 28;
    btn.heightInPixels = 24;
  ...
  private _createConsoleToggleButton(parent: BABYLON_GUI.StackPanel) {
    const btn = BABYLON_GUI.Button.CreateSimpleButton(
      "console3-button-console-toggle",
      "O"
    );
    btn.widthInPixels = 28;
    btn.heightInPixels = 24;
  ...
  private _createSideToggleButton(parent: BABYLON_GUI.StackPanel) {
    const btn = BABYLON_GUI.Button.CreateSimpleButton(
      "console3-button-side-toggle",
      ">"
    );
    btn.widthInPixels = 28;
    btn.heightInPixels = 24;
    ...
    btn.onPointerUpObservable.add(() => {
      ...
      if (this._side === Side.Left) {
        ...
        this._mainPanel.horizontalAlignment =
          BABYLON_GUI.StackPanel.HORIZONTAL_ALIGNMENT_RIGHT;
        this._buttonPanel.horizontalAlignment =
          BABYLON_GUI.StackPanel.HORIZONTAL_ALIGNMENT_RIGHT;
      } else {
        btn.rotation = 0;
      ...
      this._entities.forEach((e) => {
        if (e.options.docked) {
          const panel = existingControls?.find(
            (c) => c.name == `console3-panel-${e.name}`
          );
          if (panel) {
            if (this._side === Side.Left) {
  ...
  private _addObjectEntity(
    name: string,
    object: any,
    property: string,
    options: Console3LoggedEntityOption
  ) {
    ...
    const entity: Console3LoggedEntity = new Console3LoggedEntity(
      name,
      type,
      object,
      property,
  ...
  private _showConsole() {
    ...
    controls?.forEach((c) => {
      if (c?.name) {
        if (
          c.name.indexOf("console3-observer-panel-") > -1 ||
  ...
  private _addObserverFrame(entity: Console3LoggedEntity, inputCount = 1) {
    if (this._gui) {
      const parentPanel = new BABYLON_GUI.StackPanel();
      ...
      parentPanel.widthInPixels =
        entity.options.mapping?.widthInPixels ??
        this._options.observerPanelWidthInPixels;
      ...
      const textLabel = new BABYLON_GUI.TextBlock();
      ...
      const color =
        this._entityTypeMappings.get(entity.type)?.color ??
        this._options.observerPanelColor;
      this._setColorEntityColor(entity.name, color);
    }
  }
}
